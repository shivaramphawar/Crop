...
import pickle
import sqlite3 #for connecting the database
app=Flask(__name__)
...
def prediction():
    if request.method=='POST':
        ...
        cur=conn.cursor()
        cur.execute(f'''INSERT INTO CROP VALUES({nitro},{phos},{kp},{temp},
                    {hum},{ph},{rain},'{res[0]}')''')
        conn.commit()
...
if __name__=='__main__':
    app.run(host='0.0.0.0',port=5050) #make it for all public
